...
async def start_recap(
    ...
    thread = threading.Thread(
        target=_run_recap,
        args=(job_id, url, voice, chunk_duration, model),
        daemon=True,
    )
    thread.start()
    return {"job_id": job_id, "status": "started"}
...
def _run_recap(job_id, url, voice, chunk_duration, model):
    job = jobs[job_id]
    output_dir = os.path.join(OUTPUT_DIR, job_id)
    os.makedirs(output_dir, exist_ok=True)
    ...
    try:
        from downloader import download_video
        from transcriber import transcribe
        from hindi_recap import generate_recap
        ...
        def recap_progress(pct, msg=""):
            # Map recap progress (0-100) to job progress (30-95)
            job["progress"] = 30 + int(pct * 0.65)
            job["step"] = msg
            if msg:
                job["logs"].append(msg)

        job["step"] = "Generating Hindi narration..."
        job["logs"].append("Starting Hindi recap generation...")

        output_file = os.path.join(output_dir, "recap_hindi.mp4")
        ...
        job["clips"] = [{
            "filename": "recap_hindi.mp4",
            "url": f"/output/{job_id}/recap_hindi.mp4",
            "score": 0,
            "text": f"Hindi recap: {result['scenes']} scenes, {result['duration']}s",
            "start": 0,
            "end": result["duration"],
        ...
        job["step"] = "Done!"
        job["status"] = "completed"
        job["logs"].append(f"Done! Hindi recap: {result['scenes']} scenes, {result['duration']}s")

    except Exception as e:
        tb = traceback.format_exc()
        logger.error(f"Recap job {job_id} failed: {e}\n{tb}")
        job["status"] = "failed"
        job["error"] = str(e)
        job["logs"].append(f"Error: {e}")
